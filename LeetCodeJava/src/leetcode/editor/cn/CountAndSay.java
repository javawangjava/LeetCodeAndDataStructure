...
<div><div>Related Topics</div><div><li>字符串</li></div></div><br><div><li>👍 912</li><li>👎 0</li></div>
*/
...
class Solution {
	...
	// 使用双指针形成长度可变的滑动窗口，将字符串中连续相同的字符移入滑动窗口，当完全进入滑动窗口后，
    public String countAndSay(int n) {
		...
		String str="1";//只有一个1时
		for (int i = 2; i <= n; i++) {//i表示当前数的大小
			StringBuilder strBu=new StringBuilder();//可变字符串
			int left=0;
			int right=0;
			while(right<str.length()){//右指针遍历上一个字符串
				while(right<str.length()&&str.charAt(right)==str.charAt(left)){//当新加滑动窗口的字符与滑动窗口最左侧的字符相同时，滑动窗口右指针不断右移
					right++;
				}
				strBu.append(right-left).append(str.charAt(left));//将连续相同字符的个数和相同字符连接在可变数组后面
				left=right;//滑动窗口左指针指向每次遇到的第一个不同的字符
			}
			str= strBu.toString();//跟新上一个字符串
		}
		return str;
    }
	...
	public String countAndSay(int n) {
		...
		for (int i = 2; i <=n ; i++) {
			...
			while(right< str.length()){
				...
				str=str+(right-left)+str.charAt(left);
				left=right;
			}
		}
		return str;
